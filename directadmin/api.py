...
class ApiConnector(object):
    ...
    def execute(self, cmd, parameters=None, get=None):
        ...
        headers = {'user-agent': 'python_directadmin'}
        payload = []
        res = ""
        if get is not None: #for get : data example = [('user','pook')]
            #payload = json.dumps(get)
            payload = dict(get)
            res = requests.get(url, auth=HTTPBasicAuth(self._username, self._password), params= payload ,headers=headers)
            return (res.text)
            #return res.json();
        if parameters is not None: #for post 
            #payload = json.dumps(parameters)
            payload = dict(parameters)
            print(payload)
            res = requests.post(url, auth=HTTPBasicAuth(self._username, self._password), data= payload ,headers=headers)
            return (res.text)
            #return res.json();
        else:
            #both are none using get
            res = requests.get(url, auth=HTTPBasicAuth(self._username, self._password) ,headers=headers)
            return (res.text)
            #return res.json();
        if (res == ""):
            return "not get/post to server!!"

    def execute_original(self, cmd, parameters=None, get=None): #using post method
        """Execute command
...
class Api(object):
    ...
    def create_domain(self, domain):
        ...
        Implements command CMD_API_DOMAIN
        ...
        parameters = [('action', 'create'),
                      ...
                      ('cgi','ON'),
                      ('php','ON')]
        return self._execute_cmd("CMD_API_DOMAIN", parameters)
